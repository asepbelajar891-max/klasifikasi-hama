...
# Aturan 1: VETO (Kartu Merah Langsung)
# Jika salah satu dari ini terdeteksi dengan keyakinan cukup, gambar PASTI ditolak.
VETO_KEYWORDS = [
# Manusia & Hewan
'person', 'face', 'man', 'woman', 'hand', 'hair', 'cat', 'dog', 'animal',
# Teknologi & Kantor
'computer', 'keyboard', 'screen', 'monitor', 'mouse', 'laptop', 'remote_control', 'book',
# Pakaian & Kain
'jean', 'shirt', 'clothing', 'fabric', 'textile', 'shoe', 'wig', 'hat',
# Kendaraan
'car', 'vehicle', 'wheel', 'truck',
# Benda Dalam & Luar Ruangan
'building', 'table', 'chair', 'desk', 'wall', 'floor', 'ceiling', 'bottle', 'cup', 'shop', 'iron', 'wardrobe', 'medicine_chest'
]

# Aturan 2: APPROVAL (Tanda Pengenal)
# Jika lolos VETO, salah satu dari ini HARUS ada agar gambar diterima.
ALLOWED_KEYWORDS = [
#Bagian Tanaman & Tanaman Umum
'leaf', 'plant', 'flower', 'vine', 'garden', 'vegetable', 'foliage', 'stem', 'branch',
# Lokasi & Wadah
'espalier', 'greenhouse', 'pot', 'planter',
# Jenis Sayur/Buah yang Mungkin Mirip
'cucumber', 'artichoke', 'cardoon','broccoli', 'cauliflower', 'zucchini', 'cabbage', 'tomato', 'pomegranate', 'bell_pepper', 'strawberry', 'eggplant',
# Kategori Umum
'fruit', 'produce',
# Makhluk Kecil di Daun
'slug', 'ladybug', 'caterpillar', 'insect'
]
...
def is_image_a_leaf(image_path):
    """
    Menggunakan ResNet50 dengan logika hibrida (VETO + APPROVAL).
    """
    ...
    try:
        ...
        # Cetak prediksi untuk debugging
        print(f"Gatekeeper Predictions: {[(p[1], f'{p[2]*100:.2f}%') for p in decoded_predictions]}")

        # --- Aturan 1: Pemeriksaan VETO ---
        for _, label, confidence in decoded_predictions:
            if confidence > 0.30: # Keyakinan harus > 30% untuk VETO
                for keyword in VETO_KEYWORDS:
                    if keyword in label.lower():
                        print(f"VETO RULE TRIGGERED: Keyword '{keyword}' in label '{label}' with {confidence*100:.2f}% confidence. REJECTING.")
                        return False

        # --- Aturan 2: Pemeriksaan APPROVAL ---
        for _, label, confidence in decoded_predictions:
            if confidence > 0.05: # Keyakinan > 5% sudah cukup untuk approval
                for keyword in ALLOWED_KEYWORDS:
                    if keyword in label.lower():
                        print(f"APPROVAL RULE TRIGGERED: Keyword '{keyword}' in label '{label}'. ACCEPTING.")
                        return True

        # --- Aturan 3: Default Tolak ---
        print("DEFAULT REJECT: Image was not vetoed, but no allowed keywords were found.")
        return False
